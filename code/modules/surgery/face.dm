...
//////////////////////////////////////////////////////////////////
//	facial skin slitch surgery step
//////////////////////////////////////////////////////////////////
/datum/surgery_step/face/slitching
	allowed_tools = list(/obj/item/weapon/needle = 100)

	min_duration = 70
	max_duration = 100

/datum/surgery_step/face/slitching/can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	return ..() && target.op_stage.face > 0

/datum/surgery_step/face/slitching/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	user.visible_message("[user] is starting to sew up incison on \the [target]'s face and neck with \the [tool]." , \
	"You start to sew up incison on \the [target]'s face and neck with \the [tool].")
	..()

/datum/surgery_step/face/slitching/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("<span class='notice'>[user] slitched \the [target]'s face and neck with \the [tool].</span>", \
	"<span class='notice'>You stitched \the incision on the [target]'s face and neck with \the [tool].</span>")
	affected.open = 0
...
/datum/surgery_step/face/slitching/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("<span class='warning'>[user]'s hand slips, piercing the [target]'s face with \the [tool]!</span>", \
	"<span class='warning'>Your hand slips, piercing the [target]'s face with \the [tool]!</span>")
